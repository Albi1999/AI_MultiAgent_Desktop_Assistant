...
STREAMLIT_STYLE = """
<style>
    /* Highlight the stop button in Maggioli-blue */
    button[kind=header] {
    ...
    button[kind=header]:hover {
        background-color: #002080; /* Tonalità più scura al passaggio del mouse */
    }
...
INTERRUPT_TEXT = "(l'utente ha interrotto e ha scritto quanto segue)"
INTERRUPT_TOOL_ERROR = "l'utente ha interrotto l'esecuzione dello strumento"
...
def _reset_model_conf():
    model_conf = (
        MODEL_TO_MODEL_CONF.get(
            st.session_state.model, CLAUDE_4
        )  # Default fallback
    )
...
async def main():
    ...
    st.image("/home/computeruse/maggioli_logo.svg", width=75)
    st.title("AI Desktop Assistant")
    #st.markdown("<h1 style='text-align: center; color: grey;'>AI Desktop Assistant</h1>", unsafe_allow_html=True)  
    ...
    with st.sidebar:
        ...
        st.text_area(
            ...
            help="Aggiungi qui istruzioni extra che l'AI seguirà per task specifici.",
    ...
    new_message = st.chat_input(
        "Cosa vuoi che l'Assistente AI faccia oggi?"
    )
...
def _render_message(
    ...
    with st.chat_message(sender):
        if is_tool_result:
            ...
            for tool_id, tool in st.session_state.tools.items():
                if tool is message:
                    st.markdown(f"**Tool usato:** `{tool_id.split('-')[0]}`")
                    break
